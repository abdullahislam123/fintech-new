...
const Dashboard = () => {
  ...
  return (
    <div className="dashboard-container" style={{ minHeight: '100vh', padding: '10rem 6rem 4rem', position: 'relative' }}>
      ...
      <header style={{ marginBottom: '6rem', maxWidth: '1400px', margin: '0 auto 6rem', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <motion.div>
            ...
            <motion.h1 
              ...
              style={{ fontSize: '6rem', fontWeight: 900, letterSpacing: '-5px', marginBottom: '1.5rem', color: 'white', lineHeight: 0.9 }}
            >
              ${totalBalance.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
              <span style={{ color: 'var(--text-muted)', fontSize: '4rem' }}>.{Math.round((totalBalance % 1) * 100).toString().padStart(2, '0')}</span>
            </motion.h1>
            
            {/* Visual Action Points from target image */}
            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '2.5rem' }}>
               <motion.div 
                 whileHover={{ scale: 1.05, y: -5 }}
                 onClick={() => setModalType('deposit')}
                 className="glass-card" 
                 style={{ padding: '0.75rem 2.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem', borderRadius: 'var(--pill-radius)', border: '1px solid var(--accent-teal-soft)', cursor: 'pointer' }}
               >
                  <div style={{ background: 'var(--accent-teal)', padding: '0.6rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FaPlus size={16} color="black" /></div>
                  <span style={{ fontWeight: 800, fontSize: '1.125rem' }}>Deposit</span>
               </motion.div>
               ...
               <motion.div 
                 ...
                 className="glass-card" 
                 style={{ padding: '0.75rem 2.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem', borderRadius: 'var(--pill-radius)', border: '1px solid var(--glass-border)', cursor: 'pointer', opacity: 0.7 }}
               >
                  <div style={{ background: '#ff5555', padding: '0.6rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FaMinus size={16} color="white" /></div>
                  <span style={{ fontWeight: 800, fontSize: '1.125rem' }}>Withdraw</span>
               </motion.div>
            </div>
          </motion.div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4rem' }}>
             <div style={{ textAlign: 'right' }}>
                <div style={{ color: 'var(--accent-teal)', fontWeight: 900, fontSize: '2.5rem', letterSpacing: '-1px' }}>+12.4%</div>
                <div style={{ color: 'var(--text-muted)', fontSize: '0.8125rem', fontWeight: 800, letterSpacing: '1px' }}>PERFORMANCE (30D)</div>
             </div>
             <div style={{ position: 'relative', width: '150px', height: '150px' }}>
                ...
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', color: 'white', fontWeight: 900, fontSize: '1.25rem' }}>MIX</div>
             </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        {/* Core Protocol Stats */}
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2.5rem', marginBottom: '6rem' }}>
          {[
            ...
            { label: 'Custody Mode', icon: <FaWallet size={28} color="var(--accent-teal)" />, value: 'NON-CUSTODIAL' }
          ].map((stat, i) => (
            <motion.div 
              key={i}
              whileHover={{ translateY: -10, borderColor: 'var(--accent-teal-soft)' }}
              className="glass-card" 
              style={{ padding: '3rem 2.5rem', display: 'flex', alignItems: 'center', gap: '2rem' }}
            >
              <div style={{ background: 'rgba(0, 245, 212, 0.08)', padding: '1.25rem', borderRadius: '1.5rem', border: '1px solid rgba(0, 245, 212, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {stat.icon}
              </div>
              <div>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.8125rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '0.5rem' }}>{stat.label}</p>
                <h3 style={{ fontSize: '1.75rem', fontWeight: 900, color: 'white', letterSpacing: '-1px' }}>{stat.value}</h3>
              </div>
            </motion.div>
          ))}
        </section>
